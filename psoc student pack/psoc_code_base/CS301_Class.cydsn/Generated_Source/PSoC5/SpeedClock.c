...
/*******************************************************************************
* Function Name: SpeedClock_Start
********************************************************************************
*
* Summary:
...
void SpeedClock_Start(void) 
{
    ...
    SpeedClock_CLKEN |= SpeedClock_CLKEN_MASK;
	SpeedClock_CLKSTBY |= SpeedClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SpeedClock_Stop
********************************************************************************
*
* Summary:
...
void SpeedClock_Stop(void) 
{
    ...
    SpeedClock_CLKEN &= (uint8)(~SpeedClock_CLKEN_MASK);
	SpeedClock_CLKSTBY &= (uint8)(~SpeedClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SpeedClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SpeedClock_CLKEN & SpeedClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SpeedClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SpeedClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SpeedClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SpeedClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SpeedClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SpeedClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SpeedClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SpeedClock_GetSourceRegister();
    uint16 oldDivider = SpeedClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SpeedClock_CLKEN & SpeedClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SpeedClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SpeedClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SpeedClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SpeedClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SpeedClock_MOD_SRC |= modeBitMask & (uint8)SpeedClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SpeedClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SpeedClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SpeedClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SpeedClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SpeedClock_MOD_SRC & (uint8)(SpeedClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SpeedClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SpeedClock_GetDividerRegister();
    uint8 oldSrc = SpeedClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SpeedClock_MOD_SRC |= CYCLK_SSS;
        SpeedClock_MOD_SRC =
            (SpeedClock_MOD_SRC & (uint8)(~SpeedClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SpeedClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SpeedClock_MOD_SRC & SpeedClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SpeedClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SpeedClock_PHASE = clkPhase & SpeedClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SpeedClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SpeedClock_PHASE & SpeedClock_PHASE_MASK;
}
